...
| `FR-ADAPT-008` | `AC-ADAPT-003` | `SG-HOOK-001` | Declared desktop adapter registration surfaces | `RISK-004` | None |
...
34a. As a repository maintainer, I want each adapter to declare the client configuration file it registers in, that file's block schema, and whether that schema is independently versioned, so that activation, health, and removal work across clients that register differently without the Gate branching on a client name. (`FR-ADAPT-008`, `SG-HOOK-001`, `AC-ADAPT-003`)
...
| Adapter conformance Interface | Git maps authoritative authorization to commit behavior; supported desktop adapters normalize triggers, register through their own declared registration surfaces, and present the same structured result as non-authoritative preflight; release fixtures prove declared capabilities and portability. | `AC-EVAL-001`, `AC-ADAPT-001`, `AC-ADAPT-002`, `AC-ADAPT-003`, `AC-PORT-001` | Existing structured CLI result conventions plus the accepted cross-client capability and compatibility fixtures |
...
| `AC-ADAPT-003` | Adapters declaring different registration files and block schemas register, reconcile, and remove through their own declarations without a client-name branch; an unrelated entry in the same client configuration file survives removal; and an unconfirmable declared surface is `unverified`. | Adapter conformance Interface |
